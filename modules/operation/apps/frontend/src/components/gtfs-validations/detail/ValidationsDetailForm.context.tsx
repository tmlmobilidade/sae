...
export const ValidationsDetailContextProvider = ({ children }: PropsWithChildren) => {
	...
	const { data: fileResponse, error: fileSwrError, isLoading: fileLoading } = useSWR<ApiResponse<Attachment>>(API_ROUTES.operation.GTFS_VALIDATIONS_DETAIL_FILE(gtfsValidationId), {
	...
	const updateProcessingStatus = useCallback(async (status: ProcessingStatus) => {
		...
		try {
			const response = await fetchApiData<GtfsValidation>({
				...
				method: 'PUT',
				url: API_ROUTES.operation.GTFS_VALIDATIONS_DETAIL_PROCESSING_STATUS(gtfsValidationId),
			});
	...
	}, [gtfsValidationId, validationMutate, validationsListMutate]);
